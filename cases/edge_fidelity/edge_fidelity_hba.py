...
from build_graph_fidelity import construct_graph
from build_graph_fidelity import check_and_update_graph
...
l_argv = sys.argv
l_argv.pop(0)
temp_seed = l_argv.pop()
f_seed = int(temp_seed)
fid = l_argv.pop()
fidelity = float(fid)
l_demand = list()
for i in l_argv:
...
graph = construct_graph(UG,fidelity,f_seed) # graph = (node,graph) => node = graph[0], graph => graph[1]
#for n in graph[1]:
...
for d in met_demands:
    c_ok = check_and_update_graph(graph,d,rsc_demand_order)
...
r_value = half_based_rounding(vars,lp_vars)    # perform rounding
print(r_value)
...
for d in temp_met_demands:
    if d[0] in num_met_demands: continue
    c_ok = check_and_update_graph(graph,d,rsc_demand_order)
    if c_ok:
        met_demands.append(d)
        num_met_demands.append(d[0])
